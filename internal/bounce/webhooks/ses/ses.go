...
var regCertURL = regexp.MustCompile(`(?i)^https://sns\.[a-z0-9\-]+\.amazonaws\.com(\.cn)?/SimpleNotificationService\-[a-z0-9]+\.pem$`)

// notif is an individual notification wrapper posted by SNS.
type notif struct {
...
// New returns a new SES instance.
func New() *SES {
	return &SES{
...
func (s *SES) buildSignature(n notif) []byte {
	var b bytes.Buffer
...
func (s *SES) verifyNotif(n notif) error {
...
func (s *SES) getCert(certURL string) (*x509.Certificate, error) {
	...
	if !regCertURL.MatchString(certURL) {
